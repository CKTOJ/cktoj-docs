...
Next, we will set up the database using MariaDB. The CKTOJ is only tested to work with MySQL, and it is unlikely to work with anything else. Please visit [the MariaDB site](https://mariadb.org/download/?t=repo-config) and follow the download instructions.
...
Now that you are done, you can start installing the site. First, create a virtual environment and activate it. Here, we'll create a virtual environment named `cktsite`.

```shell-session
$ python3 -m venv cktsite
$ . cktsite/bin/activate
```

You should see `(cktsite)` prepended to your shell. Henceforth, `(cktsite)` commands assume you are in the code directory, with the virtual environment active.
...
```shell-session
(cktsite) $ git clone --recursive https://github.com/CKTOJ/OJ.git site
(cktsite) $ cd site
```
...
You will now need to configure `dmoj/local_settings.py`. You should make a copy of [this sample settings file](https://github.com/CKTOJ/cktoj-docs/blob/master/sample_files/local_settings.py) and read through it, making changes as necessary. Most importantly, you will want to update MariaDB credentials.
...
ckt uses `sass` and `autoprefixer` to generate the site stylesheets. ckt comes with a `make_style.sh` script that may be run to compile and optimize the stylesheets.
...
The ckt uses Celery workers to perform most of its heavy lifting, such as batch rescoring submissions. We will use Redis as its broker, though note that other brokers that Celery supports will work as well.
...
```shell-session
(cktsite) $ ./manage.py loaddata navbar
(cktsite) $ ./manage.py loaddata language_small
(cktsite) $ ./manage.py loaddata demo
```
...
```shell-session
(cktsite) $ ./manage.py createsuperuser
```
...
```shell-session
(cktsite) $ ./manage.py runserver 0.0.0.0:8000
```
...
```shell-session
(cktsite) $ celery -A dmoj_celery worker
```
...
First, copy our `uwsgi.ini` ([link](https://github.com/CKTOJ/cktoj-docs/blob/master/sample_files/uwsgi.ini)). You should change the paths to reflect your install.

You need to install `uwsgi`.

```shell-session
(cktsite) $ pip3 install uwsgi
```

To test, run:

```shell-session
(cktsite) $ uwsgi --ini uwsgi.ini
```
...
Copy our `site.conf` ([link](https://github.com/CKTOJ/cktoj-docs/blob/master/sample_files/site.conf)) to `/etc/supervisor/conf.d/site.conf`, `bridged.conf` ([link](https://github.com/CKTOJ/cktoj-docs/blob/master/sample_files/bridged.conf)) to `/etc/supervisor/conf.d/bridged.conf`, `celery.conf` ([link](https://github.com/CKTOJ/cktoj-docs/blob/master/sample_files/celery.conf)) to `/etc/supervisor/conf.d/celery.conf` and fill in the fields.
...
You should copy the sample `nginx.conf` ([link](https://github.com/CKTOJ/cktoj-docs/blob/master/sample_files/nginx.conf)), edit it and place it in wherever it is supposed to be for your nginx install.
...
```shell-session
(cktsite) $ pip3 install websocket-client
```

Now copy `wsevent.conf` ([link](https://github.com/CKTOJ/cktoj-docs/blob/master/sample_files/wsevent.conf)) to `/etc/supervisor/conf.d/wsevent.conf`, changing paths, and then update supervisor and nginx.
